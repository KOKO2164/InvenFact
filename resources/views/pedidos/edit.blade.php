...
@section('content_header')
    <div class="row">
        ...
        <div class="col d-flex justify-content-end">
            ...
            <a href="{{ route('pedidos.pdf', $item->id) }}" class="ml-2 btn btn-danger" target="_blank">
                    ...
                    @if ($item->detallePedidos->count() > 0)
                        ...
                        @foreach ($item->detallePedidos as $detallePedido)
                            ...
                            <tr>
                                ...
                                <td><input type="number" class="form-control" value="{{ $detallePedido->cantidad }}"
                                        onchange="actualizarDetallePedido(event)"></td>
...
@section('js')
    <script>
        ...
        function mostrarProductos(productos) {
                ...
                productos.forEach(function(producto) {
                    template += `
                        <tr>
                            ...
                            <td><input type="number" class="form-control" value="${producto.detalle.cantidad}" onchange="actualizarDetallePedido(event)"></td>
                            <td>${producto.producto.precio}</td>
                            <td>${producto.detalle.cantidad * producto.producto.precio}</td>
                            <td class="d-none">${producto.producto.id}</td>
                            <td><button type="button" class="btn btn-danger" onclick="eliminarDetallePedido(event, ${producto.producto.id})"><i class="fas fa-trash"></i></button></td>
                        </tr>
                    `;
                });
                template += `
            ...
            $('#tablaDetallesPedidos').html(template);
        }

        function actualizarDetallePedido(e) {
            e.preventDefault();
