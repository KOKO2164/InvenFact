        ...
        <div class="row">
            @foreach ($productos as $producto)
                <div class="col-xl-2 col-lg-4 col-md-4 col-sm-6 col-12 col-custom-5" style="margin-bottom: 20px;">
                    <div class="producto"
                        style="background: #343a4054; border-radius: 5px; transition: transform 0.3s ease; height: auto; position: relative; display: flex; flex-direction: column;">
                        ...
                        <span
                            style="font-size: 14px; position: absolute; top: 8px; left: 8px; background: white; color: black; border-radius: 6px; font-weight: 600;"
                            class="px-2">
                            {{ $producto->categoria->nombre }}
                        </span>
                        <span
                            style="font-size: 14px; position: absolute; top: 39px; left: 8px; background: white; color: black; border-radius: 6px; font-weight: 600;"
                            class="px-2">
                            Stock: {{ $producto->stock }}
                        </span>
                        <span
                            style="font-size: 14px; position: absolute; top: 69px; left: 8px; background: white; color: black; border-radius: 6px; font-weight: 600;"
                            class="px-2">
                            Ubicación: {{ $producto->codigoUbicacion }}
                        </span>

                        <span
                            style="font-size: 14px; position: absolute; top: 120px; right: 8px; background: rgb(241, 137, 0); color: rgb(255, 255, 255); border-radius: 6px; font-weight: 600;"
                            class="px-2">
                            s/.{{ $producto->precio }}
                        </span>
                        ...
                        <div style="display: flex; border-radius: 10px;">
                            ...
                            @if ($producto->estado)
                                <form action="{{ route('productos.disable', $producto) }}" method="POST" style="flex: 1;"
                                    class="d-inline">
                                    @csrf
                                    @method('PATCH')
                            ...
                            @else
                                <form action="{{ route('productos.enable', $producto) }}" method="POST" style="flex: 1;"
                                    class="d-inline">
                                    @csrf
                                    @method('PATCH')
